...
//        getDbInfo()
        List<AuxField> auxFields = getAuxFields();
        List<RdbDbInfo> dbInfo = getDbInfo();

        return new Rdb(auxFields, dbInfo);
    }
    ...
    private List<RdbDbInfo> getDbInfo() {
        ...
        while (bytes.get(pos) == REDIS_DB_SELECTOR) {
            ...
            pos += 2;
            RdbEncodingMetadata hashTableSizeMetadata = getEncodingMetadata(pos);
            int hashTableSize = hashTableSizeMetadata.value();
            RdbEncodingMetadata expireHashTableSizeMetadata = getEncodingMetadata(pos + hashTableSizeMetadata.encoding().getSize());
            int expireHashTableSize = expireHashTableSizeMetadata.value();
    ...
    private RdbEncodingMetadata getEncodingMetadata(int pos) {
        ...
        int flag = sizeByte >> 6;
        return switch (flag) {
            case 0 -> new RdbEncodingMetadata(RdbEncoding.LENGTH_SHORT, sizeByte & 63);
